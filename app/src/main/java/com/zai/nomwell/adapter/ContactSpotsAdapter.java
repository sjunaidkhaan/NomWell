 ...
 */
public class ContactSpotsAdapter extends BaseSwipeAdapter<MySpotsHolder> {
    ...
    public ContactSpotsAdapter(ArrayList<MySpotsData> mySpotsData, boolean showCheck, OnRecyclerViewClickListener clickListener) {
    ...
    public MySpotsHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MySpotsHolder(LayoutInflater.from(parent.getContext())
                .inflate(R.layout.list_contact_spot, null), showCheck, clickListener);
    }
    ...
    public void onBindViewHolder(MySpotsHolder holder, int position) {
        ...
        if (msd.icon == R.drawable.ic_add_white_36dp) {
            holder.imvwIcon.setColorFilter(
                    ContextCompat.getColor(holder.itemView.getContext(), R.color.blue_filter));
        } else {
            holder.imvwIcon.setColorFilter(
